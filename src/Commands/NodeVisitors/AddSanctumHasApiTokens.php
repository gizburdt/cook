...
class AddSanctumHasApiTokens extends NodeVisitorAbstract
{
            ...
            foreach ($node->stmts as $stmt) {
                if ($stmt instanceof TraitUse) {
                    foreach ($stmt->traits as $trait) {
                        if ($trait->toString() === 'HasApiTokens') {
                            $this->hasApiTokensTrait = true;
                        }
                    }
                }
            }
    ...
    protected function addTraitUse(Class_ $node): void
    {
        if ($this->hasApiTokensTrait) {
            return;
        }
        ...
        $traitToAdd = new TraitUse([new Name('HasApiTokens')]);

        if ($lastTraitIndex !== null) {
            array_splice($node->stmts, $lastTraitIndex + 1, 0, [$traitToAdd]);
        } else {
